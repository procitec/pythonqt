...
static QStringList getOperatorCodes(const AbstractMetaClass* cls) {
  ...
  QSet<QString> r;
  for(QString op: QStringList(operatorCodes.begin(), operatorCodes.end())) {
  ...
  QStringList result = QStringList(r.begin(), r.end());
...
void SetupGenerator::generate()
{
  ...
  std::stable_sort(classes_with_polymorphic_id.begin(), classes_with_polymorphic_id.end(), class_less_than);
  ...
  while (pack.hasNext()) {
    ...
    if (list.isEmpty())
      continue;
    std::stable_sort(list.begin(), list.end(), class_less_than);
...

    {
      ...
      s << "void PythonQt_init_" << shortPackName << "(PyObject* module) {" << Qt::endl;
      ...
      foreach (const AbstractMetaClass *cls, list) {
        ...
        if (cls->isQObject()) {
          s << "PythonQt::priv()->registerClass(&" << cls->qualifiedCppName() << "::staticMetaObject, \"" << shortPackName <<"\", PythonQtCreateObject<PythonQtWrapper_" << cls->name() << ">" << shellCreator << ", module, " << operatorCodes <<");" << Qt::endl;
        } else {
          QString baseName = cls->baseClass()?cls->baseClass()->qualifiedCppName():"";
          s << "PythonQt::priv()->registerCPPClass(\""<< cls->qualifiedCppName() << "\", \"" << baseName << "\", \"" << shortPackName <<"\", PythonQtCreateObject<PythonQtWrapper_" << cls->name() << ">" << shellCreator << ", module, " << operatorCodes <<");" << Qt::endl;
        }
      ...
      QStringList list = QStringList(listRegistration.begin(), listRegistration.end());
      list.sort();
