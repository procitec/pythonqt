...
class XMLParser {
public:
    XMLParser(const QString& filename, TypeDatabase *database, bool generate)
    : m_filename(filename), m_database(database), m_generate(generate ? TypeEntry::GenerateAll : TypeEntry::GenerateForSubclass)
    {
        m_current_enum = 0;
        current = 0;
...
bool XMLParser::parse()
{
    ...
    xml.readNext();
    auto tokenType = xml.tokenType();
    while (tokenType != QXmlStreamReader::NoToken && tokenType != QXmlStreamReader::Invalid) {
        if (tokenType == QXmlStreamReader::StartElement) {
            auto tagName = QString(xml.name().toLatin1());
            ...
            if (element->type & StackElement::TypeEntryMask) {
                ...
                switch (element->type) {
                    case StackElement::PrimitiveTypeEntry: {
                        ...
                        type->setPreferredConversion(
                                convertBoolean(preferred_conversion, "preferred-conversion", true));
                        type->setPreferredTargetLangType(
                                convertBoolean(preferred_java_type, "preferred-java-type", true));
                    ...
                    case StackElement::EnumTypeEntry: {
                        ...
                        m_current_enum->setForceInteger(
                                convertBoolean(attributes["force-integer"], "force-integer", false));
                        m_current_enum->setExtensible(
                                convertBoolean(attributes["extensible"], "extensible", false));
                        ...
                        if (!attributes["flags"].isEmpty() && attributes["flags"].toLower() != "no") {
                            ...
                            if (QStringList(lst.mid(0, lst.size() - 1)).join("::") !=
                    ...
                    case StackElement::ValueTypeEntry: {
                        ...
                        ctype->setGenericClass(
                                convertBoolean(attributes["generic-class"], "generic-class", false));
                        ...
                        ctype->setCreateShell(
                                convertBoolean(attributes["create-shell"], "create-shell", false));
                        ctype->setCreatePromoter(
                                convertBoolean(attributes["create-promoter"], "create-promoter", false));

                        ctype->setIsPolymorphicBase(
                                convertBoolean(attributes["polymorphic-base"], "polymorphic-base", false));
                        ...
                        if (element->type == StackElement::InterfaceTypeEntry ||
                            ...
                            if (convertBoolean(attributes["delete-in-main-thread"], "delete-in-main-thread",
                                               false))
                ...
                switch (element->type) {
                    ...
                    case StackElement::LoadTypesystem: {
                        ...
                        if (!m_database->parseFile(name,
                                                   convertBoolean(attributes["generate"], "generate", true))) {
                    ...
                    case StackElement::ConversionRule: {
                        ...
                        TypeSystem::Language lang = languageNames.value(languageAttribute,
                    ...
                    case StackElement::ModifyArgument: {
                        ...
                        argumentModification.reset_after_use = convertBoolean(
                                attributes["invalidate-after-use"], "invalidate-after-use", false);
                    ...
                    case StackElement::DefineOwnership: {
                        ...
                        TypeSystem::Ownership owner = ownershipNames.value(ownershipAttribute,
                    ...
                    case StackElement::ModifyFunction: {
                        ...
                        mod.modifiers |= (convertBoolean(attributes["virtual-slot"], "virtual-slot", false)
                                          ? Modification::VirtualSlot : 0);
                    ...
                    case StackElement::Replace:
                        ...
                        element->parent->value.templateInstance->addReplaceRule(attributes["from"],
                                                                                attributes["to"]);
                        break;
                    default:
                        break; // nada
    ...
    if( xml.error() ) {
        qWarning(qPrintable(xml.errorString()));
    }
    return !xml.error();
}

void XMLParser::fetchAttributeValues(const QString &name, const QXmlStreamAttributes &atts,
                                     QHash<QString, QString> *acceptedAttributes)
{
    Q_ASSERT(acceptedAttributes != 0);

    for (auto attribute: atts) {
        QString key = QString(attribute.name().toLatin1()).toLower();
        QString val = QString(attribute.value().toLatin1()).toLower();
...
bool XMLParser::endElement(const QString &localName)
{
...
bool XMLParser::importFileElement(const QXmlStreamAttributes &atts)
{
    auto fileName = QString(atts.value("name").toLatin1());
    if(fileName.isEmpty()){
    ...
    QFile file(fileName);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        file.setFileName(QString(":/trolltech/generator/").append(fileName));
        if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    auto quoteFrom = atts.value("quote-after-line");
    bool foundFromOk = quoteFrom.isEmpty();
    bool from = quoteFrom.isEmpty();

    auto quoteTo = atts.value("quote-before-line");
...
bool XMLParser::convertBoolean(const QString &_value, const QString &attributeName, bool defaultValue)
{
...
bool TypeDatabase::parseFile(const QString &filename, bool generate)
{
    ...
    XMLParser parser(filename, this, generate);
