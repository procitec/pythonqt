...
void ShellHeaderGenerator::write(QTextStream &s, const AbstractMetaClass *meta_class)
{
  ...
  if (meta_class->generateShellClass() && !ctors.isEmpty()) {
    ...
    s << Qt::endl;
    s << "   ~" << shellClassName(meta_class) << "();" << Qt::endl;
    s << Qt::endl;
    ...
    if (meta_class->isQObject()) {
      s << "  const QMetaObject* metaObject() const;" << Qt::endl;
      s << "  int qt_metacall(QMetaObject::Call call, int id, void** args);" << Qt::endl;
    }
  ...
  if (!promoteFunctions.isEmpty()) {
    ...
    std::stable_sort(enums1.begin(), enums1.end(), enum_lessThan);
  ...
  std::stable_sort(fields.begin(), fields.end(), field_lessThan);
