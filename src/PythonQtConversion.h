...
PyObject* PythonQtConvertListOfValueTypeToPythonList(const void* /*QList<T>* */ inList, int metaTypeId)
{
  ...
  static const int innerType = PythonQtMethodInfo::getInnerTemplateMetaType(QByteArray(QMetaType(metaTypeId).name()));
...
{
  ListType* list = (ListType*)inList;
  static PythonQtClassInfo* innerType = PythonQt::priv()->getClassInfo(PythonQtMethodInfo::getInnerListTypeName(QByteArray(QMetaType(metaTypeId).name())));
  if (innerType == NULL) {
...
{
  ListType* list = (ListType*)inList;
  static int innerType = PythonQtMethodInfo::getInnerTemplateMetaType(QByteArray(QMetaType(metaTypeId).name()));
  if (innerType == QVariant::Invalid) {
